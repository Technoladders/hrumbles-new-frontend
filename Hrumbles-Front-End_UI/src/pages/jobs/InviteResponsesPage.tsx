...
// Updated: added candidate_search as a recognised invite_source.
...
// ─── Source badge config — add new sources here ───────────────────────────────
const SRC: Record<string, { label: string; bg: string; color: string }> = {
...
function ChannelBadge({ channel }: { channel: string }) {
  const cfg: Record<string, { icon: React.ReactNode; bg: string; color: string; label: string }> = {
    ...
    <div className="space-y-6 animate-fade-in p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          {job && <p className="text-gray-500 text-sm mt-0.5">{job.title}</p>}
        </div>
          ...
          {stats.total > 0 && (
            <Card className="p-4 bg-purple-50 border border-purple-100 flex flex-col justify-center gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-purple-800 font-semibold">Conversion</span>
              </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* Source filter — now includes candidate_search */}
        <Select value={srcFilter} onValueChange={v => { setSrcFilter(v); setCurrentPage(1); }}>
          <SelectTrigger className="w-40 rounded-full bg-gray-100 border-transparent text-gray-600 text-sm h-10">
                ...
                {paginated.map(inv => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Source */}
                      <td className="px-4 py-3">
                        <SrcBadge source={inv.invite_source} />
                      </td>
