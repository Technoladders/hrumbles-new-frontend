...
export const LatestEmploymentResult = ({ result, meta }: { result: any; meta?: any }) => {
  // Extract latest employment from correct nested path
  const uanDataArray = result?.data?.uan_data;
  const latestEmp = uanDataArray?.[0]?.latest_employment_data;

  if (!latestEmp || !uanDataArray || uanDataArray.length === 0) {
    const message = result?.data?.message || 'No latest employment record found.';
    return (
  ...
  const uan = uanDataArray[0]?.uan || meta?.inputValue || 'N/A';
    ...
    <Card className="bg-green-50 border-green-200 shadow-sm">
      <CardContent className="p-5">
        ...
        <div className="mb-4 pb-3 border-b border-green-100">
          <p className="flex items-center gap-center gap-2 text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <div className="md:col-span-2 flex items-center gap-2 flex">
