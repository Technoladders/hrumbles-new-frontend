    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center mb-6">
        <Button 
          ...
          size="icon" 
          onClick={onBack} 
          className="mr-2 text-gray-500 hover:bg-gray-100"
        >
          <ArrowLeft />
        </Button>
      ...
      <div className="flex items-center space-x-2 pt-2">
        <Checkbox 
          id={`terms-${verificationType}`} 
          checked={agreed} 
          onCheckedChange={(c) => setAgreed(!!c)} 
        />
        <label 
          htmlFor={`terms-${verificationType}`} 
          className="text-sm text-gray-600"
        >
          I agree this data is shared with informed consent...
        </label>
      </div>
      
      <Button 
        onClick={() => onVerify(verificationType)} 
