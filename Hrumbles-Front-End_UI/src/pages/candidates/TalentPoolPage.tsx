...
const TalentPoolPage: FC = () => {
  ...
  const { data: candidates = [], isLoading, refetch } = useQuery<TalentPoolCandidate[]>({
    queryKey: ['talentPoolCandidates', role, user?.id],
    queryFn: async () => {
      let query = supabase.from('hr_talent_pool').select(`*, created_by:hr_employees!hr_talent_pool_created_by_fkey (first_name, last_name)`);
      if (role === '' && user?.id) query = query.eq('created_by', user.id);
