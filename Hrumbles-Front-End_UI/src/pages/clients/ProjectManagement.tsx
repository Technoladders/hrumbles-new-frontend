...
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis,Area,  ComposedChart, CartesianGrid, Tooltip, Legend } from "recharts";
...
// This effect resets the page to 0 whenever the user clicks a new view button
useEffect(() => {
  setCurrentPage(0);
}, [chartView]);
...
// =======================================================================
 ...
 if (loadingProjects || loadingEmployees || loadingTimeLogs) {
  ...
  if (!projectFinancials || projectFinancials.length === 0) {
    // This part runs if, after loading, the project data is still empty.
    // We will still render the page, but show a message inside the chart card.
    // For now, let's just log it to be sure. A full UI message is better later.
    console.log("No project data available to display in the chart.");
    // You could return a full "No Data" page here if you want:
    // return <div>No projects found for the selected date range.</div>;
  }
