...
import { useCompanyFilters } from "@/hooks/use-company-filters"; // ✅ NEW IMPORT
import { AdvancedFilterPanel, ActiveFilter } from '@/components/sales/ AdvancedFilterPanel'; // ✅ NEW IMPORT
import {
  Edit, Building2, Users, Search, Plus, Globe, Linkedin,
...
const CompaniesPage = () => {
    ...
    const { filters, isLoading: isFiltersLoading } = useCompanyFilters({
      organizationId,
      fileId,
    });
    ...
    // ✅ MODIFIED: Added activeFilters to dependency array
    useEffect(() => { 
      setCurrentPage(1); 
    }, [searchTerm, selectedCreatorId, activeFilters]);
    ...
    // ✅ MODIFIED: Updated filtering logic to include activeFilters
    const tableFilteredData = useMemo(() => {
        return companies.filter(company => {
            ...
            // ✅ NEW: Apply active filters
            const filtersMatch = activeFilters.length === 0 || activeFilters.every((filter) => {
              if (filter.values.length === 0) return true;
              
              switch (filter.filterId) {
                case 'industry':
                  return filter.values.includes(company.industry || '');
                case 'stage':
                  return filter.values.includes(company.stage || '');
                case 'location':
                  return filter.values.includes(company.location || '');
                case 'status':
                  return filter.values.includes(company.status || '');
                case 'account_owner':
                  return filter.values.includes(company.account_owner || '');
                case 'created_by':
                  const createdById = company.created_by || 'system';
                  return filter.values.includes(createdById);
                default:
                  return true;
              }
            });
            
            return searchMatch && creatorMatch && filtersMatch;
        });
    }, [companies, searchTerm, selectedCreatorId, activeFilters]); // ✅ Added activeFilters
    ...
    const processCsvData = async (data: Record<string, any>[]) => {
        ...
        data.forEach((rawRow, index) => {
            ...
            if (validationResult.success) {
                if (validationResult.data.name?.trim()) { 
                  ...
                  validationErrors.push({ row: index + 2, errors: ["Company Name is required."] }); 
                }
            } else {
        ...
        if (validationErrors.length > 0) {
            ...
            toast({ 
              title: `Validation Failed for ${validationErrors.length} rows`, 
              description: `Errors: ${errorSummary}${validationErrors.length > 3 ? '...' : ''}`, 
              variant: "destructive", 
              duration: 10000 
            });
        ...
        if (validCompanies.length > 0) {
            ...
            try {
                ...
                const { error: upsertError, count } = await supabase
                  .from('companies')
                  .upsert(companiesToUpsert as any, { onConflict: 'name', ignoreDuplicates: true });
                ...
                toast({ 
                  ...
                  description: `${insertedCount} companies processed. ${skippedCount > 0 ? `${skippedCount} duplicates skipped.` : ''}` 
                });
            ...
            if (validationErrors.length === 0) {
              toast({ title: "Import Info", description: "No new data to import." });
            }
            setIsImporting(false);
        }
    };
    ...
    return (
        <div className="space-y-8 animate-fade-in p-4 md:p-6">
            {fileId && (
                <Breadcrumb spacing="8px" separator={<ChevronRightIcon className="h-4 w-4" />} className="mb-2">
                    ...
                    {isLoadingBreadcrumb ? <Spinner size="xs" /> : breadcrumbData && (
                        <>
                            <BreadcrumbItem>
                              <BreadcrumbLink as={RouterLink} to="/lists">
                                {breadcrumbData.workspaces?.name || 'Folder'}
                              </BreadcrumbLink>
                            </BreadcrumbItem>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold mb-1">
                      {fileId ? (breadcrumbData?.name || 'Loading List...') : 'Company Dashboard'}
                    </h1>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem onSelect={() => setIsAddDialogOpen(true)}>
                              <Sparkles className="mr-2 h-4 w-4" />Fetch with AI
                            </DropdownMenuItem>
                            <DropdownMenuItem onSelect={() => setIsManualAddDialogOpen(true)}>
                              <PenSquare className="mr-2 h-4 w-4" />Enter Manually
                            </DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="p-4 flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-3xl font-bold">
                      {isCountsLoading ? '...' : counts?.companies ?? 0}
                    </h3>
                  </div>
                ...
                <Card className="p-4 flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-3xl font-bold">
                      {isCountsLoading ? '...' : counts?.employees?.toLocaleString() ?? 0}
                    </h3>
                  </div>
            ...
            {/* Table and rest of the component remains the same */}
  ...
  <div className="border-b border-gray-200 bg-gray-50 p-4">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex flex-wrap items-center gap-3 flex-1 min-w-0">
        {/* Advanced Filters Panel - NOW INSIDE TABLE */}
        <AdvancedFilterPanel
          filters={filters}
        ...
        <div className="relative flex-1 min-w-[200px] max-w-[400px]">
          ...
          <Input 
            ...
            className="pl-10 h-10 w-full rounded-lg bg-white border-gray-300 text-sm" 
            value={searchTerm} 
      ...
      {/* Right side - Creator Filter & Results Count */}
      <div className="flex items-center gap-3">
        <Select value={selectedCreatorId} onValueChange={setSelectedCreatorId}>
          <SelectTrigger className="w-[180px] h-10 bg-white border-gray-300 text-sm">
        ...
        <div className="text-sm text-gray-600 font-medium whitespace-nowrap">
          {tableFilteredData.length} {tableFilteredData.length === 1 ? 'company' : 'companies'}
        </div>
      </div>
    </div>
  </div>
                                                    ...
                                                    <div className="text-xs text-muted-foreground flex items-center gap-3 mt-1">
                                                        {company.website && (
                                                          <a 
                                                            ...
                                                            title="Website"
                                                          >
                                                            <Globe className="h-3.5 w-3.5 text-gray-400 hover:text-primary" />
                                                          </a>
                                                        )}
                                                        {company.linkedin && (
                                                          <a 
                                                            ...
                                                            title="LinkedIn"
                                                          >
                                                            <Linkedin className="h-3.5 w-3.5 text-gray-400 hover:text-primary" />
                                                          </a>
                                                        )}
                                                    </div>
                                                </div>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            ...
                                            {company.employee_count_date && (
                                              <div className="text-xs text-gray-400">
                                                as of {moment(company.employee_count_date).format("MMM yyyy")}
                                              </div>
                                            )}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                     <Button 
                                                       variant="outline" 
                                                       size="sm" 
                                                       className={`h-7 px-2 text-xs w-full max-w-[150px] justify-between truncate border-0 font-semibold ${stageColors[getDisplayValue(company.stage, 'default')] ?? stageColors['default']}`} 
                                                       disabled={updateStageMutation.isPending && updateStageMutation.variables?.companyId === company.id}
                                                     >
                                                        <span className="truncate">
                                                          {getDisplayValue(company.stage, 'Select Stage')}
                                                        </span>
                                            ...
                                            <div className="flex items-center">
                                                ...
                                                <div className="ml-3">
                                                    <div className="font-medium text-gray-800">
                                                        {company.created_by_employee 
                                                          ? company.created_by_employee.first_name 
                                                          : 'System'}
                                                    </div>
                                                    <div className="text-xs text-gray-400">
                                                ...
                                                <div className="flex items-center space-x-1 rounded-full bg-slate-100 p-1 shadow-sm border border-slate-200">
                                                    <TooltipProvider>
                                                      <Tooltip>
                                                        ...
                                                        <TooltipContent><p>View Company</p></TooltipContent>
                                                      </Tooltip>
                                                    </TooltipProvider>
                                                    <TooltipProvider>
                                                      <Tooltip>
                                                        <TooltipTrigger asChild>
                                                          <Button 
                                                            variant="ghost" 
                                                            size="icon" 
                                                            onClick={() => handleEditClick(company as CompanyDetail)} 
                                                            className="h-7 w-7 rounded-full text-slate-500 hover:bg-purple-600 hover:text-white transition-colors"
                                                          >
            ...
            {tableFilteredData.length > 0 && (
                <div className="flex flex-col sm:flex-row justify-between items-center mt-4 gap-4">
                    ...
                    <span className="text-sm text-gray-600">
                        ...
                        {Math.min(currentPage * itemsPerPage, tableFilteredData.length)} of{' '}
                        {tableFilteredData.length} companies
                    </span>
                </div>
            )}
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>Fetch Company Data</DialogTitle>
                    <DialogDescription>Enter a company name to fetch details using AI.</DialogDescription>
                  </DialogHeader>
                  <CompanyAddForm 
                    onAdd={handleCompanyDataFetched} 
            ...
            <Dialog open={isReviewDialogOpen} onOpenChange={setIsReviewDialogOpen}>
                <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>Review and Create Company</DialogTitle>
                    <DialogDescription>Review the AI-fetched data then save.</DialogDescription>
                  </DialogHeader>
                  {companyToReview && (
                    <CompanyEditForm 
            ...
            <Dialog open={isManualAddDialogOpen} onOpenChange={setIsManualAddDialogOpen}>
                <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    ...
                    <DialogDescription>Fill out the form and click "Create Company" to save.</DialogDescription>
                  </DialogHeader>
                  <CompanyEditForm 
                    company={{}} 
            ...
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    ...
                    <DialogDescription>Update details for {editCompany?.name}.</DialogDescription>
                  </DialogHeader>
                  {editCompany && (
                    <CompanyEditForm 
                      company={editCompany} 
