...
const CompanyDetail = () => {
  ...
  // Fetch ALL contacts associated with this company (both employees and POCs)
  const { data: allContacts = [], isLoading: isLoadingContacts, error: contactsError, refetch: refetchContacts } = useQuery({
    queryKey: ['company-contacts-all', companyId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('contacts')
        .select(`
      ...
      // Transform to match CandidateDetail interface
  ...
  const handleRefreshIntelligence = async () => {
    ...
    try {
      ...
      if (company?.apollo_org_id) {
        ...
        throw new Error("Cannot sync: Missing both ID and Website/Domain.");
      }
      ...
      toast({ title: "Success", description: "Company data enriched successfully" });
      refetchCompany();
