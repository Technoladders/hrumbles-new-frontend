...
export default function TanstackContactsPage() {
  ...
  // 1. Redux State
  const { isDiscoveryMode, totalEntries, currentPage, perPage, filters } = useSelector(
  ...
  // Sync Redux -> URL (when filters/page change)
  useEffect(() => {
    if (isDiscoveryMode && Object.keys(filters).length > 0) {
  ...
  }, [filters, currentPage, isDiscoveryMode]); // Removed searchParams dependency to avoid loop
  ...
  useEffect(() => {
    if (userPreferences && !preferencesLoaded) {
      if (userPreferences.column_visibility) setColumnVisibility(userPreferences.column_visibility);
      if (userPreferences.column_order) setColumnOrder(userPreferences.column_order);
      if (userPreferences.column_sizing) setColumnSizing(userPreferences.column_sizing);
  ...
  const totalRowCount = isDiscoveryMode ? totalEntries : (queryResult?.count || 0);

  const isPendingFilterChange = isFetching && !isLoading;

  const saveContactAvailability = async (contactId: string, person: any) => {
    try {
      const hasEmail = !!person.email || (Array.isArray(person.emails) && person.emails.length > 0) || !!person.has_email;
      const hasPhone = (Array.isArray(person.phone_numbers) && person.phone_numbers.length > 0) || !!person.mobile_phone || !!person.sanitized_phone || person.has_direct_phone === 'Yes';
      const hasLocation = !!person.city || !!person.state || !!person.country || !!person.has_city || !!person.has_country;
  ...
  const handleEnrich = async (contactId: string, apolloId: string | null, type: 'email' | 'phone', personDetails?: any) => {
    try {
      ...
      const { data, error } = await supabase.functions.invoke('enrich-contact-master', { body });
      if (error) throw new Error(error.message || "Function invocation failed");
      toast({ title: "Success", description: data?.message || "Enrichment processing" });
      queryClient.invalidateQueries({ queryKey: ['contacts-unified'] });
    } catch (err: any) {
  ...
  const handleListAdd = async (targetFileId: string) => {
    ...
    try {
      if (isFromDiscovery && selectedContact?.original_data) {
        ...
        const savedContact = await saveDiscoveryToCRM(person, organization_id, user.id);
        if (!savedContact?.id) throw new Error("Contact was created but no ID returned");
        await saveContactAvailability(savedContact.id, person);
        await supabase.from('contact_workspace_files').upsert({
          contact_id: savedContact.id,
        ...
        toast({ title: "Success", description: `${person.first_name} saved to CRM and added to list.` });
        queryClient.invalidateQueries({ queryKey: ['contacts-unified'] });
        queryClient.invalidateQueries({ queryKey: ['listRecordCounts'] });
  ...
  useEffect(() => {
    if (isDiscoveryMode) {
      setColumnVisibility(prev => ({
        ...prev, email: false, mobile: false, contact_stage: false, medium: false, 
        created_by_employee: false, created_at: false, updated_at: false, location: false, data_availability: true,
      }));
    } else {
      setColumnVisibility(prev => ({
        ...prev, email: true, mobile: true, contact_stage: true, medium: true, 
        created_by_employee: true, created_at: true, updated_at: false, location: true, data_availability: false,
      }));
    }
  }, [isDiscoveryMode]);
  ...
  const table = useReactTable({
    ...
    onPaginationChange: (updater) => {
      ...
      if (newPagination.pageSize !== perPage) {
        dispatch(setPerPage(newPagination.pageSize));
        dispatch(setPage(1));
  ...
  const getColumnDisplayName = (columnId: string) => columnId.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  const pageTitle = fileId && currentFile ? currentFile.name : isDiscoveryMode ? 'Global Intelligence' : 'My Contacts';
  const recordCount = isDiscoveryMode ? totalEntries : table.getFilteredRowModel().rows.length;
      ...
      <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-slate-50">
        ...
        <header className="bg-white border-b px-6 py-3 flex items-center justify-between shadow-sm z-30 flex-shrink-0">
          <div className="flex items-center gap-4">
             {fileId && (
               <Button variant="ghost" size="sm" onClick={() => navigate('/lists')} className="text-slate-600 hover:text-slate-900">
                ...
                <div className="flex items-center gap-2">
                  {fileId && currentFile?.workspaces && (
                    <Badge variant="outline" className="text-[9px] bg-slate-100">
                      <FolderOpen size={10} className="mr-1" /> {currentFile.workspaces.name}
                    </Badge>
                  )}
          ...
          <div className="flex items-center gap-3">
             {fileId && !isDiscoveryMode && (
               <Button variant="secondary" size="sm" onClick={() => setIsImportOpen(true)} className="h-9 text-xs font-semibold bg-purple-50 text-purple-700 hover:bg-purple-100 border border-purple-200">
                 <UploadCloud size={14} className="mr-2" /> Import CSV
               </Button>
             )}
             ...
             <Button variant="outline" size="sm" className="h-9 hidden lg:flex" onClick={() => setViewSettingsOpen(true)}>
               <Settings2 className="mr-2 h-3.5 w-3.5" /> View
             </Button>
             {!isSidebarOpen && (
               <Button variant="outline" size="icon" onClick={() => setIsSidebarOpen(true)} className="h-9 w-9">
                 <Filter size={14}/>
               </Button>
             )}
          </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          {isSidebarOpen && (
            <div className="w-[220px] flex-shrink-0 border-r bg-white z-20 overflow-hidden">
               {isDiscoveryMode ? (
                 <DiscoverySidebar initialFilters={filters} /> 
               ) : (
                 <ContactFiltersSidebar table={table} isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} fileId={fileId || null} />
               )}
            </div>
          )}
            ...
            <div className="flex-1 overflow-hidden p-0 pb-0 flex flex-col relative">
               {isLoading ? (
                <div className="flex-1 flex flex-col items-center justify-center">
                  ...
                  <p className="mt-4 text-xs text-slate-500 font-medium">Loading Contacts...</p>
                </div>
              ) : isPendingFilterChange ? (
                <div className="flex-1 flex flex-col items-center justify-center opacity-70">
                  <Spinner size="lg" color="purple.500" />
                  <p className="mt-4 text-xs font-black text-slate-500 uppercase tracking-widest">Applying filters...</p>
                </div>
              ) : (
                <div className={cn("flex-1 bg-white rounded-t-2xl border shadow-sm overflow-hidden", isFetching ? "opacity-60 pointer-events-none" : "opacity-100")}>
                  {tableData.length === 0 ? (
                    isDiscoveryMode ? (
                      <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
                        <div className="bg-purple-50 p-4 rounded-full mb-4"><Zap className="h-8 w-8 text-purple-600" /></div>
                        <h3 className="text-lg font-bold text-slate-800">Global Intelligence Search</h3>
                        <p className="text-sm text-slate-500 max-w-md mt-2">Search over 275M+ verified contacts.</p>
                      </div>
                    ) : (
                      <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
                        <div className="bg-slate-100 p-4 rounded-full mb-4"><FolderOpen className="h-8 w-8 text-slate-400" /></div>
                        <h3 className="text-lg font-bold text-slate-800">No Contacts Yet</h3>
                        <p className="text-sm text-slate-500 max-w-md mt-2">Start by adding contacts or searching in Discovery mode.</p>
                      </div>
                    )
                  ) : (
        ...
        <ContactImportDialog open={isImportOpen} onOpenChange={setIsImportOpen} fileId={fileId || null} />
        
        {/* View Settings Dialog (Content same as original) */}
        <Dialog open={viewSettingsOpen} onOpenChange={setViewSettingsOpen}>
           <DialogContent className="sm:max-w-[500px] max-h-[80vh]">
             <DialogHeader>
               <DialogTitle className="flex items-center gap-2"><Settings2 className="h-5 w-5" /> Table View Settings</DialogTitle>
               <DialogDescription>Customize which columns are visible, their order, and sizes.</DialogDescription>
             </DialogHeader>
             ...
             <DialogFooter className="flex justify-between mt-4">
                <Button variant="outline" onClick={handleResetPreferences} className="text-red-600 hover:text-red-700 hover:bg-red-50">
                  <RotateCcw className="h-4 w-4 mr-2" /> Reset to Defaults
                </Button>
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setViewSettingsOpen(false)}>Cancel</Button>
                  <Button onClick={() => { savePreferences(); setViewSettingsOpen(false); }} className="bg-purple-600 hover:bg-purple-700">
