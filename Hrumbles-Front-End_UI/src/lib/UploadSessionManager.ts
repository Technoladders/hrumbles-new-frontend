...
//
// ARCHITECTURE CHANGE:
//   Browser: parse file → upload to storage → insert session_file row
//   Server: talent-batch-submit reads from DB → chunks JSONL → calls OpenAI
//
// FIXES APPLIED:
//   1. Chunked submit: Breaks large sessions into 500-file chunks
//   2. Proper dedup: Checks hr_talent_pool for existing candidates
//   3. Timeout handling: AbortController with 120s timeout + clear error
//   4. No .catch() on Supabase builders
...
const CONCURRENCY        = 8;
const MAX_RETRIES        = 3;
const SUBMIT_CHUNK_SIZE  = 500;  // Chunk size for large batch submission
const SUBMIT_TIMEOUT     = 120_000; // 2 min timeout for submit
...
class UploadSessionManager {
  ...
  private async processFiles(files: File[], sessionId: string, organizationId: string): Promise<void> {
    ...
    // Use chunked submit for large sessions
    await this.submitSessionInChunks(sessionId);
  }
  ...
  private async processOneFile(file: File, sessionId: string, organizationId: string): Promise<void> {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      if (attempt > 0) {
        const backoff = Math.min(Math.pow(1.5, attempt) * 1000, 5000);
      ...
      try {
        ...
        if (existing?.length > 0) {
          ...
          const { error: dupErr } = await supabase.from('hr_talent_pool_upload_session_files').insert({
            session_id: sessionId, organization_id: organizationId,
            file_name: file.name, file_size: file.size, file_hash: fileHash,
            resume_path: prev.resume_path, resume_text: null,
            upload_status: 'skipped', error_message: `Duplicate of ${prev.file_name}`,
          });
          if (dupErr) console.warn('Failed to insert duplicate record:', dupErr);
          return;
        }
        ...
        if (file.type === 'application/pdf') {
          ...
          try {
            invokeResult = await supabase.functions.invoke('talent-pool-parser', { body: file });
          } finally {
            clearTimeout(timeout);
          }

          if (invokeResult?.error || !invokeResult?.data?.text) {
            const msg = invokeResult?.error?.message ?? 'Parser returned no text';
          ...
          text = invokeResult.data.text;
          compressedBlob = base64ToBlob(invokeResult.data.compressedBase64, 'application/pdf');
        ...
        const { error: insertErr } = await supabase.from('hr_talent_pool_upload_session_files').insert({
          session_id: sessionId, organization_id: organizationId,
          file_name: file.name, file_size: file.size, file_hash: fileHash,
          resume_path: resumePath, resume_text: text, upload_status: 'uploaded',
        });

        if (insertErr) {
          console.error('Failed to insert session file:', insertErr);
          throw new Error(`DB insert failed: ${insertErr.message}`);
        }

        // Increment count
        const { error: rpcErr } = await supabase.rpc('increment_session_uploaded_count', { p_session_id: sessionId });
        if (rpcErr) {
          console.warn('RPC failed, using fallback:', rpcErr);
          try {
            const { data: sd } = await supabase.from('hr_talent_pool_upload_sessions')
              .select('uploaded_count').eq('id', sessionId).single();
            if (sd) {
              await supabase.from('hr_talent_pool_upload_sessions')
                .update({ uploaded_count: (sd.uploaded_count ?? 0) + 1 }).eq('id', sessionId);
            }
          } catch (fbErr) { console.error('Fallback failed:', fbErr); }
        }

        updateFile({ status: 'done', resumePath });
        return;
        ...
        if (permanent || attempt >= MAX_RETRIES) {
          ...
          await supabase.from('hr_talent_pool_upload_session_files').insert({
            ...
            upload_status: 'failed', error_message: msg,
          });

          const { error: fcErr } = await supabase.rpc('increment_session_failed_count', { p_session_id: sessionId });
          if (fcErr) console.warn('Failed to increment failed count:', fcErr);
          return;
        }
  ...
  // ✅ NEW: Chunked submit to avoid 150s timeout
  private async submitSessionInChunks(sessionId: string): Promise<void> {
    ...
    const totalFiles = countData?.length ?? 0;
    const totalChunks = Math.ceil(totalFiles / SUBMIT_CHUNK_SIZE);
    const batchJobIds: string[] = [];

    for (let chunk = 0; chunk < totalChunks; chunk++) {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), SUBMIT_TIMEOUT);

      try {
        this.emit({ type: 'progress', session: { 
          ...this.session!,
          files: [{ 
            id: 'chunk-info', fileName: `Submitting chunk ${chunk + 1}/${totalChunks}`, 
            fileSize: 0, status: 'uploading' 
          }]
        }});
        ...
        if (err.name === 'AbortError') {
          this.emit({ type: 'error', message: `Chunk ${chunk + 1} timed out after 120s. Retry or use smaller batches.` });
        } else {
          this.emit({ type: 'error', message: `Chunk ${chunk + 1} failed: ${err.message}` });
        }
    ...
    this.emit({ type: 'submitted', batchJobId: batchJobIds.join(',') });
    this.session = null;
  }
}
