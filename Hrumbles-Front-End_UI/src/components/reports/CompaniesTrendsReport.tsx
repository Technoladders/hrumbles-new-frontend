// src/components/reports/CompaniesTrendsReport.tsx
...
const CompaniesTrendsReport: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const { data, error: dataError } = await supabase.from('companies')
          .select('created_at, hr_employees!companies_created_by_fkey (id, first_name, last_name)')
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Company Creation Trends</CardTitle><p className="text-sm text-muted-foreground">Analyze company creation activity with detailed breakdowns.</p></CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3"><Card><CardHeader className="flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium">Total Companies Added</CardTitle><Sigma className="h-4 w-4 text-muted-foreground"/></CardHeader><CardContent><div className="text-2xl font-bold">{totalAdded}</div><p className="text-xs text-muted-foreground">in selected period</p></CardContent></Card><Card><CardHeader className="flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium">Peak Activity</CardTitle><ArrowUp className="h-4 w-4 text-muted-foreground"/></CardHeader><CardContent><div className="text-2xl font-bold">{peak.count}</div><p className="text-xs text-muted-foreground">on {peak.label !== 'N/A' && granularity === 'daily' ? format(new Date(peak.label), 'MMM d, yyyy') : peak.label}</p></CardContent></Card><Card><CardHeader className="flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium">Average Additions</CardTitle><Activity className="h-4 w-4 text-muted-foreground"/></CardHeader><CardContent><div className="text-2xl font-bold">{average}</div><p className="text-xs text-muted-foreground">per {granularity.slice(0,-2)}</p></CardContent></Card></div>
        ...
        <Card>
            ...
            <CardContent>
                <div className="h-[300px] w-full">{isLoading ? <div className="flex h-full w-full items-center justify-center"><LoadingSpinner/></div> : chartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">{granularity === 'daily' ? (<AreaChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="label" tickFormatter={formatTick} /><YAxis allowDecimals={false}/><Tooltip /><Area type="monotone" dataKey="count" name="Companies" stroke="#8884d8" fill="#8884d8" fillOpacity={0.3} /></AreaChart>) : (<BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="label" tickFormatter={formatTick} /><YAxis allowDecimals={false}/><Tooltip /><Legend /><Bar dataKey="count" name="Companies Added" fill="#8884d8" /></BarChart>)}</ResponsiveContainer>
