...
export interface BreakLog {
  ...
  break_type: string; // 'coffee' | 'lunch' | etc.
}
...
const AttendanceReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      ...
      try {
        ...
        setEmployees(data.map((e: any) => ({ id: e.id, name: `${e.first_name} ${e.last_name}` })));
      } catch (err: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch time_logs with nested break_logs in one query
        const { data, error } = await supabase
          .from('time_logs')
          .select(`
            *,
        ...
        const formattedData: TimeLog[] = data.map((log: any) => ({
          ...
          // Attach break_logs array; default to empty array if none
  ...
  const tabTriggerClass =
    'px-4 py-1.5 rounded-full text-sm font-medium text-gray-600 dark:text-gray-300 data-[state=active]:bg-violet-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all';
          ...
          <div className="flex flex-wrap items-center justify-start gap-3 md:gap-4 w-full mb-6">
            ...
            <div className="flex-shrink-0 order-1">
              <TabsList className="inline-flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 p-1 shadow-inner space-x-0.5">
                <TabsTrigger value="daily" className={tabTriggerClass}>
                  Daily Attendance
                </TabsTrigger>
                <TabsTrigger value="monthly" className={tabTriggerClass}>
                  Monthly Attendance
                </TabsTrigger>
                <TabsTrigger value="monthly-inout" className={tabTriggerClass}>
                  Monthly In-Out
                </TabsTrigger>
                <TabsTrigger value="monthly-break" className={tabTriggerClass}>
                  Break Report
                </TabsTrigger>
              </TabsList>
            </div>
                ...
                <PopoverContent className="w-64 p-2">
                  <div className="flex flex-col gap-2 max-h-60 overflow-y-auto">
                    ...
                    {employees.map((e) => (
                      <Label
                        key={e.id}
                        className="flex items-center gap-2 p-2 rounded hover:bg-muted font-normal"
                      >
                        <Checkbox
