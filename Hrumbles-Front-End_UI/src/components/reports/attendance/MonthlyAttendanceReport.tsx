// src/components/reports/attendance/MonthlyAttendanceReport.tsx
...
const MonthlyAttendanceReport: React.FC<MonthlyAttendanceReportProps> = ({ data, employees, selectedMonth }) => {
  ...
  const { headers, rows, totals } = useMemo(() => {
    ...
    const totals = headers.map(h => logs.filter(l => format(new Date(l.date), 'd') === h.label).length);
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {headers.map(h => (
                    <TableHead key={h.label} className={cn("text-center", isWeekend(h.date) && "text-muted-foreground")}>{h.label}</TableHead>
                  ))}
