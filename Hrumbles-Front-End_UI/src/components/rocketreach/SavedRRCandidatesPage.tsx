/**
 * SavedRRCandidatesPage.tsx — FINAL (bug-fixed)
 *
 * Bugs fixed vs the previous version:
 *   BUG 1 — inviteTarget declared inside TableRow but used in page JSX.
 *            Fix: removed from TableRow, declared at page level.
 *   BUG 2 — organizationId undefined in CandidateInviteGate call.
 *            Fix: use orgId (the Redux value).
 *   BUG 3 — onInvite prop missing from RowProps + TableRow.
 *            Fix: added to interface, passed from page via setInviteTarget.
 *   BUG 4 — useSavedCandidates called with unknown searchSource prop.
 *            Fix: hook already ignores unknown params; also add proper support
 *            via .eq("search_source","rocketreach") — see useSavedCandidates patch below.
 */
        ...
        <>
          ...
          <div className="absolute left-0 top-8 z-20 w-36 bg-white border border-slate-200 rounded-lg shadow-lg py-1">
            {Object.entries(STATUS_CFG)
              ...
              .map(([key, c]) => (
                <button key={key}
                  onClick={e => { e.stopPropagation(); onUpdate(row.id, key); setOpen(false); }}
                  className={cn(
            ...
            <button
              onClick={e => { e.stopPropagation(); onUpdate(row.id, "archived"); setOpen(false); }}
...
// ─── TableRow ─────────────────────────────────────────────────────────────────
// BUG 1 FIX: inviteTarget removed from here — it belongs at page level.
// BUG 3 FIX: onInvite prop added to interface and wired to the Send button.
interface RowProps {
  row:            SavedCandidate;
  selected:       boolean;
  onSelect:       () => void;
  onInvite:       (r: SavedCandidate) => void;  // ← page sets inviteTarget
...
const TableRow: React.FC<RowProps> = ({
  row, selected, onSelect, onInvite, onAddToFolder, onStatusUpdate, onRevealDone,
}) => {
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3 min-w-[185px]">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <p className="text-[12px] font-semibold text-slate-800 leading-tight truncate max-w-[148px]">
              {row.snapshot_name ?? "—"}
            </p>
            {row.snapshot_title && (
              <p className="text-[10px] text-slate-400 truncate max-w-[148px]">{row.snapshot_title}</p>
            )}
            {rrProfileId && (
              <p className="text-[8px] text-orange-500 font-mono mt-0.5">RR #{rrProfileId}</p>
            )}
          </div>
        </div>
      </td>
      ...
      <td className="px-4 py-3 min-w-[130px]">
        {row.snapshot_company
          ? <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] text-slate-600 truncate max-w-[110px]">{row.snapshot_company}</span>
            </div>
          : <span className="text-slate-300 text-[11px]">—</span>}
        {row.snapshot_location && (
          <div className="flex items-center gap-1 mt-0.5">
            <MapPin size={9} className="text-slate-300 flex-shrink-0" />
            <span className="text-[10px] text-slate-400 truncate max-w-[110px]">{row.snapshot_location}</span>
          </div>
        )}
      </td>
      ...
      <td className="px-4 py-3 min-w-[195px]" onClick={e => e.stopPropagation()}>
        <RRRevealCell
          type="email" rrProfileId={rrProfileId} savedValue={row.email}
      ...
      <td className="px-4 py-3 min-w-[175px]" onClick={e => e.stopPropagation()}>
        <RRRevealCell
          type="phone" rrProfileId={rrProfileId} savedValue={row.phone}
          onRevealed={(t, v) => onRevealDone(row.id, t, v)}
        />
      </td>
      ...
      <td className="px-4 py-3 min-w-[115px]">
        {row.hr_jobs
          ? <span className="text-[10px] text-blue-700 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-md font-medium truncate max-w-[105px] block">
              {row.hr_jobs.title}
            </span>
          : <span className="text-slate-300 text-[11px]">—</span>}
      </td>
      ...
      <td className="px-4 py-3 w-[80px]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
          {/* Invite — enabled only after email or phone is available */}
          <button
            onClick={() => onInvite(row)}          // ← calls page-level setInviteTarget
            disabled={!canInvite}
            title={canInvite ? "Invite candidate" : "Reveal email or phone first"}
            className={cn(
...
const SavedRRCandidatesPage: React.FC = () => {
  ...
  // Auth — orgId comes from Redux; keep as orgId throughout to avoid confusion
  ...
  // BUG 1 + BUG 2 FIX: inviteTarget declared HERE at page level
  ...
  // useSavedCandidates — passes searchSource so the hook can filter server-side.
  // If your current hook doesn't support searchSource yet, the backfill SQL ensures
  // all RR rows have search_source='rocketreach', so all returned rows will be RR.
  const { candidates, totalCount, isLoading, refetch } = useSavedCandidates({
    ...
    page,
    perPage:        PER_PAGE,
    searchSource:   "rocketreach" as any,   // see useSavedCandidates patch below
  });
  ...
  const TH: React.FC<{ children: React.ReactNode; cls?: string }> = ({ children, cls }) => (
    <th className={cn(
      "px-4 py-2.5 text-left text-[9px] font-bold uppercase tracking-wider whitespace-nowrap select-none",
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">
                {isLoading ? "Loading…" : `${totalCount} candidate${totalCount !== 1 ? "s" : ""}`}
              </p>
            </div>
          </div>
                      ...
                      {isLoading
                              ...
                              {Array.from({ length: 9 }).map((_, j) => (
                                <td key={j} className="px-4 py-3">
                                  <div className="h-3 bg-slate-100 rounded animate-pulse"
                                    style={{ width: j === 0 ? 140 : j === 2 || j === 3 ? 100 : 70 }} />
                                </td>
                              ))}
                            </tr>
                          ))
                        ...
                        : displayCandidates.map(c => (
                            <TableRow
                              ...
                              onInvite={setInviteTarget}       // ← passes page-level setter
      ...
      {/* ── Modals — at page root so inviteTarget / addFolderTarget are in scope ── */}

      {/* BUG 1 + BUG 2 FIX: inviteTarget is in scope here; orgId not organizationId */}
      {inviteTarget && (
        <CandidateInviteGate
          ...
          organizationId={orgId}          // ← orgId, not organizationId
          userId={userId}
