  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-2 bg-background/80 backdrop-blur-sm border rounded-md shadow-lg">
        ...
        <p className="text-primary">{`Contacts Created: ${payload[0].value}`}</p>
      </div>
    );
  }
  return null;
...
const CreatorPerformanceChart: React.FC<CreatorPerformanceChartProps> = ({ data }) => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Creator Performance</CardTitle>
        <CardDescription>Number of contacts added by each user in the selected period.</CardDescription>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            ...
            <Bar dataKey="companies_created" name="Contacts Created" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
