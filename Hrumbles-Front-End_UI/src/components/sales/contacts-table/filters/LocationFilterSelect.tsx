...
// City colour matches the rose theme already used in ContactFiltersSidebar
...
/**
 * Search across countries, states, and (when query ≥ 3 chars) cities.
 * The `allowedTypes` param lets each instance restrict which result types
 * to surface (e.g. type="country" only shows countries).
 */
function searchLocations(
  ...
  if (!q) {
    ...
    if (!allowedTypes.includes('country')) return [];
  ...
  if (allowedTypes.includes('country')) {
    ALL_COUNTRIES
  ...
  if (allowedTypes.includes('state')) {
    ALL_STATES
  ...
  // Cities are expensive (~150 k rows) — only scan with 3+ chars
  if (allowedTypes.includes('city') && q.length >= 3) {
    City.getAllCities()
...
function PortalDropdown({
  anchorRef, isOpen, options, query, allowedTypes, onSelect,
}: PortalDropdownProps) {
  ...
  // Show only legend items relevant to this instance
  const legendTypes = (Object.entries(LOC_STYLE) as [LocationType, typeof LOC_STYLE[LocationType]][])
  ...
  return ReactDOM.createPortal(
    <div style={pos} className="bg-white border border-slate-200 rounded-lg shadow-2xl overflow-hidden">
      ...
      <div className="max-h-[220px] overflow-y-auto overscroll-contain">
        {options.length === 0 ? (
          <p className="px-3 py-3 text-[11px] text-slate-400 italic text-center">
            {query.length > 0 && query.length < 3 && allowedTypes.includes('city')
      ...
      {!query && allowedTypes.includes('country') && (
...
interface LocationFilterSelectProps {
  ...
  /**
   * 'country' → shows only countries
   * 'city'    → shows states + cities  (no countries, for the City sub-filter)
   * 'all'     → shows countries, states and cities  (default)
   */
...
export const LocationFilterSelect: React.FC<LocationFilterSelectProps> = ({
  ...
  // Derive which location types to search based on `type` prop
  const allowedTypes = useMemo((): LocationType[] => {
    if (type === 'country') return ['country'];
    if (type === 'city')    return ['state', 'city'];
  ...
  const options = useMemo(
    () => searchLocations(query, selectedLocations, allowedTypes),
    [query, selectedLocations, allowedTypes],
  );
  ...
  const placeholder =
    ...
    type === 'city'    ? 'Search states or cities…' :
  ...
  return (
    <div ref={wrapperRef} className="space-y-2">
      ...
      {selectedLocations.length > 0 && (
        <div className="flex flex-wrap gap-1 max-h-[68px] overflow-y-auto pb-0.5">
          {selectedLocations.map(val => {
            const locType = getLocationType(val);
            const style   = LOC_STYLE[locType];
            return (
              <span
                key={val}
                className={cn(
      ...
      {/* Input row — anchorRef lives here for portal positioning */}
      <div className="relative" ref={anchorRef}>
        ...
        {/* Portal — rendered at document.body */}
        <PortalDropdown
          anchorRef={anchorRef}
          isOpen={isOpen}
          options={options}
      ...
      {!query && selectedLocations.length === 0 && (
        <p className="text-[9px] text-slate-400">
          {type === 'country' ? 'Search any country' :
           type === 'city'    ? 'Search any state or city' :
