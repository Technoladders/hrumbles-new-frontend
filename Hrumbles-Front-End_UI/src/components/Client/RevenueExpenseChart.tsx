          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="90%" height={250}>
              <ComposedChart data={monthlyData}>
                  ...
                  <YAxis yAxisId="left" orientation="left" stroke="#7B43F1" tickFormatter={(value) => `₹${(value / 100000).toFixed(0)}L`} />
                  <YAxis yAxisId="right" orientation="right" stroke="#10b981" tickFormatter={(value) => `₹${(value / 100000).toFixed(0)}L`} />
                  <RechartsTooltip formatter={(value: number) => `₹${value.toLocaleString()}`} />
                  <Legend />
                  
                  {/* Area for Profit (drawn first, in the background) */}
                  <Area yAxisId="right" type="monotone" dataKey="profit" name="Profit" fill="#10b981" stroke="#10b981" fillOpacity={0.6} />

                  {/* Bars for Revenue (drawn second, on top) */}
                  <Bar yAxisId="left" dataKey="revenue" name="Revenue" fill="#7B43F1" radius={[4, 4, 0, 0]} />
              </ComposedChart>
            </ResponsiveContainer>
          )}
      </CardContent>
