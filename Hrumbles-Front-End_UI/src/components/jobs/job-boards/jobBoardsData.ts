...
export type BoardStatus = "available" | "partial" | "not_configured" | "coming_soon";
...
export interface JobBoard {
  ...
  setupUrl:        string;
  setupNote:       string;
  integrationMode: "xml_feed" | "api_push" | "manual";
}

// Master XML feed URL — single URL for all clients, all boards that use XML
export const MASTER_FEED_URL = "https://kbpeyfietrwlhwcwqhjw.supabase.co/functions/v1/talent-xml-feed";
...
export const JOB_BOARDS: JobBoard[] = [
  {
    ...
    tags: ["Free", "AI Matching", "Global", "XML Feed"],
    setupUrl: "https://in.talent.com/integrations",
    setupNote: "Share the master XML feed URL below with your Talent.com account manager (leon@talent.com). This single URL covers all client organisations — no need to register new feeds when clients onboard.",
    integrationMode: "xml_feed",
    configFields: [
      {
        key: "feed_url", label: "Master XML Feed URL (share with Talent.com)",
        type: "readonly", placeholder: "", required: false,
        helpText: "One URL for all clients. Talent.com crawls every 24–48h once activated.",
      },
    ],
  },
  {
    ...
    setupNote: "Email CareerJet to activate feed crawling. Share the master XML feed URL below — it covers all client organisations automatically.",
    integrationMode: "xml_feed",
    configFields: [
      {
        key: "feed_url", label: "Master XML Feed URL (share with CareerJet)",
        type: "readonly", placeholder: "", required: false,
        helpText: "Auto-generated from broadcast jobs across all clients.",
      },
      { key: "partner_email", label: "Partner Email", type: "email", placeholder: "hr@yourcompany.com", required: true },
    ],
  },
  {
    id: "whatjobs", name: "WhatJobs", tagline: "API-based posting · India + EU",
    logoUrl: "https://logo.clearbit.com/whatjobs.com", logoFallback: "WJ", brandColor: "#1D72B8",
    status: "available", tier: "free", tierLabel: "Free",
    region: "India + EU", monthlyVisitors: "8M", estimatedReach: 2100, avgApplicants: "1–5", indexTime: "Instant",
    tags: ["Free", "API Push", "India"],
    setupUrl: "https://www.whatjobs.com/ats/api",
    setupNote: "WhatJobs uses a direct API push — jobs are posted instantly when you broadcast. Enter your WhatJobs API token below. Contact WhatJobs at their ATS portal to obtain your x-feed-token.",
    integrationMode: "api_push",
    configFields: [
      {
        key: "api_token", label: "WhatJobs API Token (x-feed-token)",
        type: "password", placeholder: "Obtained from WhatJobs ATS portal",
        required: true, helpText: "Visit whatjobs.com/ats/api to register and get your token.",
      },
      {
        key: "contact_email", label: "Notification Email",
        type: "email", placeholder: "hr@yourcompany.com",
        required: false, helpText: "Optional — for WhatJobs to send application notifications.",
      },
    ],
  },
  {
    ...
    setupNote: "Register as an employer on Recruit.net India and submit the master XML feed URL below.",
    integrationMode: "xml_feed",
    configFields: [
      {
        key: "feed_url", label: "Master XML Feed URL",
        type: "readonly", placeholder: "", required: false,
      },
  ...
  },
  {
    ...
    status: "not_configured", tier: "freemium", tierLabel: "Freemium",
  ...
  },
  {
    ...
    status: "not_configured", tier: "premium", tierLabel: "Paid",
  ...
  },
  {
    ...
    setupUrl: "", setupNote: "Integration coming soon.",
    integrationMode: "manual",
    configFields: [],
  },
  {
...
export const XML_FEED_BOARDS = ["talent", "careerjet", "recruitnet"];
export const API_PUSH_BOARDS = ["whatjobs"];
...
export type PostHistory = Record<string, PostRecord>;
