...
// Each row: left = candidate info (name, title, company, jobs, skills, edu)
//           right = action card (match score, contact, invite, bookmark)
...
function Avatar({ name, size=38 }:{name:string;size?:number}) {
  ...
  return <div style={{width:size,height:size,borderRadius:'50%',flexShrink:0,background:bg,display:'flex',alignItems:'center',justifyContent:'center',fontSize:Math.round(size*0.36),fontWeight:700,color:fg}}>{ini}</div>;
}

function CopyBtn({ text }:{text:string}) {
  const [done,setDone] = useState(false);
  return <button type="button" onClick={e=>{e.stopPropagation();navigator.clipboard.writeText(text);setDone(true);setTimeout(()=>setDone(false),1500);}} style={{flexShrink:0,background:'none',border:'none',cursor:'pointer',padding:2,color:'#CBD5E1',display:'flex',alignItems:'center'}} onMouseEnter={e=>(e.currentTarget.style.color='#7C3AED')} onMouseLeave={e=>(e.currentTarget.style.color='#CBD5E1')}>
...
const ZxRow: FC<{
  ...
  return (
    <div style={{ background:'white', borderRadius:10, border:`1px solid ${isSelected?'#A78BFA':'#E2E8F0'}`, outline:isSelected?'2px solid #DDD6FE':'none', marginBottom:7, boxShadow:'0 1px 3px rgba(0,0,0,0.04)', transition:'all 0.1s', display:'flex', alignItems:'flex-start', gap:12, padding:'11px 14px' }}
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        {/* Meta row: company, exp, location, notice */}
        ...
        {(r.key_skills||[]).length>0&&(
          <div style={{display:'flex',flexWrap:'wrap',gap:3}}>
            {(r.key_skills||[]).slice(0,6).map(sk=>{
              ...
              return <span key={sk} style={{padding:'2px 7px',borderRadius:4,fontSize:9,fontWeight:500,border:'1px solid',whiteSpace:'nowrap',...(isMatch?{background:'#EDE9FE',color:'#6D28D9',borderColor:'#C4B5FD'}:{background:'#F8FAFC',color:'#64748B',borderColor:'#E2E8F0'})}}>
      ...
      <div style={{flexShrink:0,width:160}} onClick={e=>e.stopPropagation()}>
        <div style={{borderRadius:10,border:'1px solid #E2E8F0',background:'#FAFAFA',padding:10,display:'flex',flexDirection:'column',gap:6}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4,marginTop:2}}>
            <button onClick={()=>onInvite(r)} style={{display:'flex',alignItems:'center',justifyContent:'center',gap:4,padding:'5px 4px',borderRadius:6,border:'1px solid #C4B5FD',background:'white',fontSize:10,fontWeight:600,color:'#6D28D9',cursor:'pointer',transition:'all 0.1s'}}
              onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='#EDE9FE';}}
              onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background='white';}}>
              <Send size={9}/> Invite
            </button>
            <button onClick={()=>onBookmark(r.id,isBm)} style={{display:'flex',alignItems:'center',justifyContent:'center',gap:4,padding:'5px 4px',borderRadius:6,border:'1px solid #E2E8F0',background:'white',fontSize:10,fontWeight:600,color:isBm?'#7C3AED':'#64748B',cursor:'pointer',transition:'all 0.1s'}}
          ...
          <Link to={`/talent-pool/${r.id}`} onClick={e=>e.stopPropagation()} style={{display:'flex',alignItems:'center',justifyContent:'center',gap:4,padding:'4px',borderRadius:6,border:'1px solid #E2E8F0',background:'white',fontSize:10,fontWeight:600,color:'#64748B',cursor:'pointer',textDecoration:'none',transition:'all 0.1s'}}
...
const Skeleton = () => (
  <div style={{background:'white',borderRadius:10,border:'1px solid #E2E8F0',padding:'11px 14px',marginBottom:7,display:'flex',gap:12,alignItems:'flex-start'}}>
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',gap:6}}>
      ...
      <div style={{display:'flex',gap:4}}>{[60,50,70,48].map((w,i)=><div key={i} style={{height:18,background:'#F1F5F9',borderRadius:4,width:w,animation:'zxPulse 1.5s infinite'}}/>)}</div>
    </div>
...
interface ZiveXCardListProps {
  results:CandidateSearchResult[]; isLoading:boolean;
  highlightTerms:string[]; selectedIds:Set<string>;
  onToggleSelect:(id:string)=>void; onInvite:(c:CandidateSearchResult)=>void;
}

const ZiveXCardList: FC<ZiveXCardListProps> = ({ results, isLoading, highlightTerms, selectedIds, onToggleSelect, onInvite }) => {
      ...
      {totalPages>1&&(
        <div style={{flexShrink:0,padding:'7px 14px',background:'white',borderTop:'1px solid #F1F5F9',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:4}}>
            <button onClick={()=>setPage(p=>Math.max(p-1,1))} disabled={page===1} style={{width:28,height:28,borderRadius:6,border:'1px solid #E2E8F0',background:'white',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#64748B',opacity:page===1?0.4:1}}>
              <ChevronLeft size={13}/>
            </button>
            {Array.from({length:Math.min(totalPages,7)},(_,i)=>i+1).map(p=>(
              <button key={p} onClick={()=>setPage(p)} style={{width:28,height:28,borderRadius:6,border:'1px solid',cursor:'pointer',fontSize:11,fontWeight:600,...(p===page?{background:'#6D28D9',color:'white',borderColor:'#6D28D9'}:{background:'white',color:'#475569',borderColor:'#E2E8F0'})}}>
                {p}
              </button>
            ))}
            {totalPages>7&&<span style={{fontSize:11,color:'#94A3B8'}}>…{totalPages}</span>}
            <button onClick={()=>setPage(p=>Math.min(p+1,totalPages))} disabled={page===totalPages} style={{width:28,height:28,borderRadius:6,border:'1px solid #E2E8F0',background:'white',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#64748B',opacity:page===totalPages?0.4:1}}>
