// src/components/talent-intelligence/TIProfileModal.tsx — v3
// Changes vs v2:
//   1. RevealBlock: calls ti-reveal (not ti-reveal-contact)
//   2. RevealBlock: email/phone buttons always visible — canPersonalEmail/canPhone gates removed
//   3. RevealBlock: handles addedToWaterfall response (auto-waterfall)
//   4. RevealBlock: waterfallInQueue / waterfallDone states + realtime subscription
//   5. RevealBlock: waterfall time "30–180 min" not "24–48h"
//   6. Rest of modal (layout, experience, education, invite picker) unchanged
  ...
  if (profile.profile_picture_url && !err) {
    return (
      <img src={profile.profile_picture_url} alt={profile.full_name ?? ""}
        className={`${sz} rounded-xl object-cover border-2 border-white shadow-md`}
        onError={() => setErr(true)} />
    );
  }
  return (
    <div className={`${sz} rounded-xl bg-gradient-to-br from-violet-500 to-violet-800
...
// ── RevealBlock — v3 ──────────────────────────────────────────
// CHANGE: always visible, ti-reveal, waterfall support
...
function RevealBlock({ profile, waterfallEnabled = false, onRevealDone }: RevealBlockProps) {
      ...
      try {
        ...
        if (data?.status === "pending") {
          ...
          if (data.found_email) {
            ...
            setEmails([newEmail]);
            onRevealDone([newEmail], data.found_phone
              ? [{ number: data.found_phone, type: "unknown", recommended: true }]
              : phones);
          }
        }
      } catch (err) {
        console.error("[TIProfileModal] waterfall check:", err);
      } finally {
  ...
  const subscribeToWaterfall = () => {
    if (waterfallChannelRef.current) supabase.removeChannel(waterfallChannelRef.current);
    waterfallChannelRef.current = supabase
      .channel(`wf-modal-${profile.id}`)
      .on("postgres_changes", {
        event:  "UPDATE",
        schema: "public",
        ...
        if (payload.new?.status === "found" && payload.new.found_email) {
          ...
          const newPhones: TIRevealedPhone[] = payload.new.found_phone
            ? [{ number: payload.new.found_phone, type: "mobile", recommended: true }]
            : phones;
          ...
          setWaterfallInQueue(true); setWaterfallDone(false);
        }
      })
      .subscribe();
  };
  ...
  const startPhonePolling = (url: string) => {
    ...
    pollRef.current = setInterval(async () => {
      if (++c > 20) {
        ...
        setPhonePending(false);
        setPhoneErr("Phone verification taking longer — check back shortly.");
        return;
      }
      try {
        const { data } = await supabase
  ...
  // CHANGE 1: calls ti-reveal (not ti-reveal-contact)
  // CHANGE 2: no canPersonalEmail / canPhone gate — always callable
    ...
    try {
      const { data, error: fnErr } = await supabase.functions.invoke("ti-reveal-v1", {
        body: {
          ...
          organizationId,
          userId,
          // Pass snapshot so auto-waterfall has profile data
          snapshotName:    profile.full_name    ?? null,
          snapshotTitle:   profile.title        ?? null,
          snapshotCompany: profile.company_name ?? null,
        },
      });

      if (fnErr || (data?.error && !data?.phonePending && !data?.addedToWaterfall)) {
      ...
      // CHANGE 3: auto-waterfall response — transition to queue state
      ...
      setErr(err?.message ?? "Reveal failed.");
    } finally {
      setLoad(false);
    }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── Email block ── */}
      {/* CHANGE 2: always shown (removed canPersonalEmail gate) */}
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        <div className="flex items-center justify-between mb-2">
          ...
          {/* Show Reveal when: not yet revealed, not loading, not in queue */}
          {!emailWasRevealed && !emailLoad && !waterfallInQueue && !waterfallChecking && (
            <button onClick={() => reveal("email")}
              className="flex items-center gap-1 px-3 py-1 bg-violet-600 text-white
                text-[11px] rounded-lg font-medium hover:bg-violet-700 transition-colors">
              <Eye size={10} /> Reveal
            </button>
          )}
          {emailLoad && (
            <span className="flex items-center gap-1 text-[11px] text-violet-600">
              <Loader2 size={10} className="animate-spin" /> Revealing…
            </span>
          )}
          {waterfallChecking && (
        ...
        {/* Waterfall in-queue — CHANGE 5: 30–180 min */}
        {waterfallInQueue && !waterfallDone && (
          <div className="flex items-start gap-2.5 p-2.5 bg-amber-50 rounded-lg border border-amber-100">
            ...
            <div>
              <p className="text-[11px] font-semibold text-amber-700">Contact sourcing in progress</p>
              <p className="text-[10px] text-amber-600 mt-0.5 leading-snug">
                Our team is sourcing this contact. Will appear here automatically within
                <strong> 30–180 minutes</strong>.
              </p>
            </div>
          </div>
        )}

        {/* Waterfall done — found */}
        {waterfallDone && hasPersonal && (
          <div className="space-y-1.5">
            {personalEmails.map((e, i) => (
              <div key={i} className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <a href={`mailto:${e.email}`} onClick={ev => ev.stopPropagation()}
                    className="text-sm text-violet-700 font-medium hover:underline truncate block">
                    {e.email}
                  </a>
                  <span className="text-[10px] text-slate-400">
                    personal{e.is_primary ? " · primary" : ""}
                    {" "}· <span className="text-amber-600">via waterfall</span>
                  </span>
                </div>
        ...
        {/* Normal reveal result */}
        {!waterfallInQueue && !waterfallDone && hasPersonal && (
          <div className="space-y-1.5">
            {personalEmails.map((e, i) => (
              <div key={i} className="flex items-center justify-between gap-2">
                <div className="min-w-0">
        ...
        {/* Revealed but no personal email */}
        {noPersonalFound && (
      ...
      {/* ── Phone block ── */}
      {/* CHANGE 2: always shown (removed canPhone gate) */}
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
          ...
          {phones.length === 0 && !phonePending && (
            <button onClick={() => reveal("phone")} disabled={phoneLoad}
              className="flex items-center gap-1 px-3 py-1 border border-violet-300
                text-violet-700 text-[11px] rounded-lg font-medium hover:bg-violet-50
        ...
        {pp ? (
          ...
          <div className="flex items-start gap-2.5 p-2.5 bg-amber-50 rounded-lg border border-amber-100">
            ...
            <div>
              ...
              <p className="text-[10px] text-amber-600 mt-0.5 leading-snug">
                Locating this contact's number. Will appear here automatically within 1–2 minutes.
              </p>
            </div>
          </div>
        ) : !phoneLoad && (
  ...
  return (
    <div className="flex gap-3">
      <div className="flex flex-col items-center flex-shrink-0">
        <div className={`w-2.5 h-2.5 rounded-full border-2 mt-1 flex-shrink-0
          ${exp.is_current ? "border-violet-600 bg-violet-600" : "border-slate-300 bg-white"}`} />
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              {exp.logo_url && (
                <img src={exp.logo_url} alt={exp.company_name}
          ...
          <div className="text-right flex-shrink-0">
            ...
            {exp.is_current && (
              <span className="inline-block mt-0.5 px-1.5 py-0.5 bg-green-100 text-green-700
                text-[9px] rounded-full font-medium">Current</span>
            )}
          </div>
        </div>
        {exp.locality && (
          <p className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5">
            <MapPin size={9} />{exp.locality}
          </p>
        )}
        {exp.summary && (
          <p className="text-xs text-slate-500 mt-1.5 leading-relaxed line-clamp-3">{exp.summary}</p>
        )}
      </div>
    </div>
  );
...
// ── Inline email/phone picker for Send Invite ─────────────────
...
function EmailPickerDropdown({ personalEmails, phones, onSelect, onClose }: EmailPickerDropdownProps) {
  ...
  return (
    <div className="absolute right-0 top-full mt-1 w-60 bg-white border border-slate-200
      rounded-xl shadow-2xl z-[99999] p-3 space-y-2 animate-in fade-in zoom-in-95 duration-150">
      <p className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">
        Choose invite contact
      </p>
        ...
        <div className="space-y-0.5">
          ...
          {personalEmails.map((e, i) => (
            <label key={i} className="flex items-center gap-2 cursor-pointer py-1
              hover:bg-slate-50 rounded px-1">
              <input type="radio" name="modal-invite"
        ...
        <div className="space-y-0.5">
          ...
          {phones.slice(0, 2).map((p, i) => (
            <label key={i} className="flex items-center gap-2 cursor-pointer py-1
              hover:bg-slate-50 rounded px-1">
              <input type="radio" name="modal-invite"
                checked={sel?.kind === "phone" && sel.value === p.number}
                onChange={() => setSel({ kind: "phone", value: p.number })}
                className="accent-violet-600 flex-shrink-0" />
              <span className="text-[10px] font-mono text-slate-700">{p.number}</span>
              {p.recommended && (
                <span className="text-[8px] px-1 bg-green-100 text-green-700 rounded ml-auto">✓</span>
              )}
            </label>
          ))}
        </div>
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={onClose}
          className="flex-1 py-1.5 border border-slate-200 rounded-lg text-[11px]
            text-slate-500 hover:bg-slate-50">Cancel</button>
        <button disabled={!sel}
          onClick={() => sel && onSelect(
            sel.kind === "email" ? sel.value : null,
            sel.kind === "phone" ? sel.value : null
          )}
          className="flex-1 py-1.5 bg-violet-600 text-white rounded-lg text-[11px]
            font-semibold disabled:opacity-40 hover:bg-violet-700 transition-colors
...
function ModalContent({ profile, onClose, onInvite, onProfileUpdate, waterfallEnabled = false }: TIProfileModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-slate-50 rounded-2xl shadow-2xl w-full max-w-5xl
        max-h-[92vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <div className="relative">
              <button onClick={handleSendInvite}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-600 text-white
                  text-xs rounded-lg font-medium hover:bg-violet-700 transition-colors">
                <Send size={12} />
                Send Invite
                {(revealedPersonalEmails.length > 1 ||
                  (revealedPersonalEmails.length >= 1 && revealedPhones.length > 0)) && (
            ...
            {profile.linkedin_url && (
              <a href={profile.linkedin_url} target="_blank" rel="noopener noreferrer"
                ...
                className="flex items-center gap-1 px-3 py-1.5 border border-[#0A66C2]
                  text-[#0A66C2] text-xs rounded-lg font-medium hover:bg-blue-50 transition-colors">
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          {/* Left column */}
          <div className="w-[280px] flex-shrink-0 bg-white border-r border-slate-200
            overflow-y-auto p-5 space-y-5">
            ...
            <div className="text-center">
              <div className="flex justify-center mb-3 relative">
                ...
                {isOpenToWork && (
                  <div className="absolute bottom-0 right-[calc(50%-44px)] w-5 h-5
                    bg-green-500 rounded-full border-2 border-white flex items-center justify-center">
              ...
              {isOpenToWork && (
                <span className="inline-block mt-2 px-2.5 py-0.5 bg-green-100 text-green-700
                  text-xs rounded-full font-medium">Open to Work</span>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {profile.first_discovered_at && (
                <div className="flex items-center gap-2 text-xs text-slate-400">
                  ...
                  <span>Discovered {new Date(profile.first_discovered_at).toLocaleDateString(
            ...
            {skills.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {skills.map(sk => (
                    <span key={sk} className="px-2 py-0.5 bg-violet-50 border border-violet-200
                      text-violet-700 text-[11px] rounded-full font-medium">{sk}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {allLangs.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {allLangs.map(l => (
                    <span key={l.name} className="px-2 py-0.5 bg-slate-100 text-slate-600
                      text-[11px] rounded-full">{l.name}</span>
                  ))}
                </div>
              </div>
            )}
                ...
                <div className="space-y-3">
                  {allEdu.map((e, i) => (
                    <div key={i} className="flex gap-2">
                      <div className="w-7 h-7 rounded bg-blue-100 flex items-center
            ...
            {allCerts.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  {allCerts.map((c, i) => (
                    <div key={i} className="flex gap-2">
                      <div className="w-7 h-7 rounded bg-amber-100 flex items-center
                        justify-center flex-shrink-0">
