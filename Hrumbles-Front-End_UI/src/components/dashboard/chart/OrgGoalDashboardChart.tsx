...
import { Loader2, Target, Trophy, Users, CalendarClock } from "lucide-react";
...
export const OrgGoalDashboardChart: React.FC<OrgGoalDashboardChartProps> = ({ organizationId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: rpcData, error } = await supabase.rpc('get_org_goal_analytics', {
          ...
          group_by_type: viewMode,
          filter_goal_type: goalType // Passing the new filter
        });
    ...
    fetchData();
  }, [organizationId, dateRange, viewMode, goalType]);
      ...
      <CardContent className="pt-6 flex flex-col h-full">
        ...
        <div className="flex flex-col gap-4 mb-4">
          <div className="flex items-center justify-between">
            ...
            {/* Date Picker on top right for mobile stack, or inline for desktop */}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/50 p-2 rounded-lg border border-slate-100">
            ...
            <Tabs value={viewMode} onValueChange={setViewMode} className="w-full sm:w-auto">
              <TabsList className="inline-flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 p-1 shadow-inner space-x-0.5">
                <TabsTrigger 
                  value="goal" 
                  className="px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-300 data-[state=active]:bg-violet-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all flex items-center gap-1"
                >
            ...
            {/* Goal Type Tabs (The New Requirement) */}
                      ...
                      shape={(props) => {
                        ...
                        return (
                          <g>
                            ...
                            {/* 2. Create a clipping region that matches the bar */}
                            <clipPath id={clipId}>
                              <rect
                                x={x}
                                y={y}
                            ...
                            {/* 3. Draw the animated wave INSIDE the bar */}
                            <use
                              href="#oceanWave"
                              x={x}
                              y={y - 20}       /* sits perfectly at the bar top */
                              clipPath={`url(#${clipId})`}
                              width={width}
                              fill={gradientId}
                              opacity={0.3}
